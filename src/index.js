...
scheduler.loadFromDatabase(function(err, cronjob_list) {
  if (err) {
    logger.error("Cron jobs list is null, cannot proceeded any further.");
    process.exit(1);
  }
  else {
    logger.info("The cron jobs account ID list: ");
    logger.info(JSON.stringify(Object.keys(cronjob_list), null, 4));
          ...
          handler: function(request,reply) {
            ...
            account.save(function(err) {
              if (!err) {
                ...
                logger.info("Registering the cron job for the account: " + account.accountNumber);
                // create the cron job; this will start it at the same time
              ...
              if (err) {
                ...
                account.delete(function(err) {
                  if (err) {
                    ...
                    logger.info("Stopping and deregistering the cron job for the account: " + account.accountNumber);
                    // stop the cron job
                    cronjob_list[account.accountNumber].stop();
                    // deregister the cron job
                    delete cronjob_list[account.accountNumber];
